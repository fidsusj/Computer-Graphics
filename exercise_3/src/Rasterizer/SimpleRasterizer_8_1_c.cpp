                                                 ...
                                                 const mat4 &modelTransformNormals)
{
  ...
  // Here, use the function LightVertex. The final triangle t should be in window coordinates.
  // Since we render an image, we need to use the height and width of the image accordingly.
...
void SimpleRasterizer::RenderMesh(const Mesh *mesh)
{
  ...
  //Berechnen Sie Modelltransformation f�r das Dreiecksnetz mesh, 
...
bool SimpleRasterizer::Render(Image &image, const Scene &scene)
{
  ...
  // Here, begin with the camera transformation (viewTransform) and
